...
let mixpnl = `function getQueryParam(url, param) { param = param.replace(/[[]/, "\[").replace(/[]]/, "\]"); var regexS = "[\?&]" + param + "=([^&#]*)", regex = new RegExp( regexS ), results = regex.exec(url); if (results === null || (results && typeof(results[1]) !== 'string' && results[1].length)) { return ''; } else { return decodeURIComponent(results[1]).replace(/\W/gi, ' '); } }; function campaignParams() {var campaign_keywords = 'utm_source utm_medium utm_campaign utm_content utm_term'.split(' '), kw = '', params = {}, first_params = {}; var index; for (index = 0; index < campaign_keywords.length; ++index) { kw = getQueryParam(document.URL, campaign_keywords[index]); if (kw.length) { params[campaign_keywords[index] + ' [last touch]'] = kw; } } for (index = 0; index < campaign_keywords.length; ++index) { kw = getQueryParam(document.URL, campaign_keywords[index]); if (kw.length) { first_params[campaign_keywords[index] + ' [first touch]'] = kw; } } const clientid = navigator.userAgent.replace(/\D+/g, ''); console.log(clientid); mixpanel.people.set(clientid, params); mixpanel.people.set_once(clientid,first_params); mixpanel.register(params); mixpanel.people.set(clientid,{ 'last touch':  location.href });mixpanel.people.set_once(clientid,{ 'first touch':  location.href });mixpanel.people.increment('page_views');mixpanel.track('page_load', { current_url: location.href, $current_url: location.href, distinct_id: clientid }); mixpanel.identify; localStorage.setItem("clientid", clientid); } campaignParams();`
